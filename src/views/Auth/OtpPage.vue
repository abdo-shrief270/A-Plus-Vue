        ...
        <div class="border border-primary-200 rounded-2xl p-8 mt-32">
          ...
          <div class="flex justify-center gap-4 mb-4">
            <input
              ...
              type="text"
              maxlength="1"
              class="w-14 h-14 text-center text-2xl border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
              @input="focusNext(index)"
              @keydown.backspace="focusPrev(index)"
            />
          </div>

          <Button label="تأكيد الرمز" class="w-full mt-4" @click="submitOtp" />
          <div class="text-center mt-4">
            <p class="text-sm text-gray-600">
              لم يصلك الرمز؟
              <span class="text-primary cursor-pointer underline" @click="resendOtp"
                >إعادة الإرسال</span
              >
            </p>
          </div>
...
const focusNext = (index) => {
  if (otp.value[index].length && index < 3) {
    nextTick(() => {
...
const submitOtp = () => {
  ...
  // تحقق من الكود أو إرساله للـ API
}
