  ...
  <div class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
    ...
    <div class="hidden px-12 md:flex relative items-start justify-center bg-primary-100 overflow-hidden">
      ...
      <div class="z-[100] mt-4 w-full flex flex-col items-start justify-start">
        <div>
          <a-button label="عودة" severity="link" variant="text" icon="pi pi-arrow-right" iconPos="right"
            class="text-white" style="color: white" @click="$router.push('/')" />
        </div>
      ...
      <img :src="parentAccount ? images.parentAuth : images.girlAuth"
        class="absolute z-10 bottom-0 w-[70%] max-h-[75%] object-contain" alt="auth image" />
    </div>
      ...
      <div class="m-8 w-full max-w-xl">
        ...
        <div v-if="!parentAccount">
          ...
          <div v-if="step === 1" class="border border-primary-200 rounded-2xl p-8">
            ...
            <div class="space-y-4">
              <a-input v-model="createObj.user_name" type="text" label="اسم المستخدم" placeholder="ادخل اسمك" />
              <a-input v-model="createObj.email" type="email" label="البريد الالكتروني"
                placeholder="ادخل بريدك الالكتروني" />
              <a-password v-model="createObj.password" type="password" label="كلمة المرور"
                placeholder="ادخل كلمة المرور" />

              <a-password v-model="createObj.password_confirmation" type="password" label="تاكيد كلمة المرور"
                placeholder="تاكيد كلمة المرور" />
              <a-phone v-model="createObj.phone" v-model:code="createObj.country_code" />

              <div>
                <label class="block font-medium mb-1">الجنس</label>
                <div class="flex gap-4">
                  <Button :outlined="createObj.gender !== 'male'"
                    :severity="createObj.gender === 'male' ? 'primary' : 'secondary'" class="w-full"
                    @click="createObj.gender = 'male'">ذكر</Button>
                  <Button :outlined="createObj.gender !== 'female'"
                    :severity="createObj.gender === 'female' ? 'primary' : 'secondary'" class="w-full"
                    @click="createObj.gender = 'female'">أنثى</Button>
                </div>
              </div>

              <Button label="التالي" class="w-full mt-4" @click="step++" />
              <Button label="فتح حساب ولي أمر" outlined class="w-full text-sm text-center mt-2"
                @click="switchAccountType(true)" />
            </div>
          </div>
          ...
          <div v-else class="border border-primary-200 rounded-2xl p-8">
            ...
            <div class="space-y-4">
              <a-select v-model="createObj.goal" :options="goalOptions" label="حدد هدفك"
        ...
        <div v-else class="border border-primary-200 rounded-2xl p-8">
          ...
          <div class="space-y-4">
            ...
            <Button label="عودة لحساب الطالب" outlined class="w-full text-sm text-center mt-2"
              @click="switchAccountType(false)" />
          </div>
        </div>
      </div>
    </div>
...
const rules = route.query.type === 'student' ? {
  user_name: { required },
...
const createObj = ref(route.query.type === 'student' ? {
  name: '',
  email: '',
  password: '',
...
const switchAccountType = (isParent) => {
  parentAccount.value = isParent
  router.replace({
    query: {
      ...route.query,
...
watch(
  () => route.query.type,
  (type) => {
    parentAccount.value = type === 'parent'
  },
  { immediate: true }
)
